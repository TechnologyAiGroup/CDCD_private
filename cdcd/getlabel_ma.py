...
def makelabel(chip, fault):
    ...
    for dictory in os.listdir(picroot):
        id_resp_fail = int(dictory.split("_")[0])

        # 获取golden candidates
        with open(root + f"{id_resp_fail}_fail" + "/all.diag", "r") as f:
            lines = f.readlines()
        f.close()
        golden = analysis(lines=lines)
        if not golden:
            print(f"golden has no fault! in {id_resp_fail}_fail")
            cmd = f"rm -r ./pic/{chip}/{fault}/{id_resp_fail}_resp"
            os.system(cmd)
            continue
            # exit()

        # 初始化candidates和labels，用于存储每个diag文件的结果和对应图片的标签
        candidates = [-1] * (len(os.listdir(root + f"{id_resp_fail}_fail")) - 1)
        # labels = [-1] * (len(os.listdir(root + dictory)) - 1)
        labels = [-1] * len(candidates)
        haveall = 0
        ...
        # 分析非all.diag的diag文件
        for file_name in os.listdir(root + f"{id_resp_fail}_fail"):
            if file_name == "all.diag":
                ...
                with open(root + f"{id_resp_fail}_fail" + "/" + file_name, "r") as f:
                    lines = f.readlines()
                f.close()
                ...
                if labels[id_diag] == -1:
                    if len(intermediate) == 0:
                        # 定义之外的情况
                        print(os.path.join(root+f"{id_resp_fail}_fail", file_name))
                        labels[id_diag] = 0
                    else:
